...
# check first argument (tag name) is valid - v#.#.#-classic or v#.#.#-bcc
if ! [[ "$1" =~ ^(v[1-9][0-9]{0,2})(\.[0-9]{1,3}){2,3}-(classic|bcc)$ ]]; then
...
result=$(curl -s "https://api.github.com/repos/brodyreeves/PartyMarker/tags" | grep -Po '"name":.*?[^\\]",' | grep -cP "\"$1\"")
